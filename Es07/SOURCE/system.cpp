...
void System :: gibbs(int i){
  double delta_E, acceptance;
  delta_E = 2.0 * 
            ( _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H );   /*probabilità che lo spin sia 1*/
  acceptance = 1./ (1+exp(-_beta*delta_E)); /*probabilità che lo spin della particella non venga flippato va da 0 a acceptance*/
  if(_rnd.Rannyu() > acceptance ) _particle(i).setspin(-1); /*se rnd è maggiore di acceptance sono nel segmento da acceptance a 1 ergo spin è -1*/
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ifstream Seed("../RandomGenerator/seed.in");
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8*M_PI*_npart*_rho*((1./(9*pow(_r_cut, 9)))-(1./(3*pow(_r_cut, 3))));
...
void System :: measure(){ // Measure properties
  ...
  double pressure_temp=0.0; /*per calcolo pressione*/
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        // GOFR 
        if(_measure_gofr){
          int bin = int(dr/_bin_size);    /*indice del bin in cui cade dr considerando il primo bin=0 cioè index_gofr, poi gli altri a seguire fino a bin=99*/
          if(bin>=0 and bin<=99) _measurement(_index_gofr + bin)+=2; /*considero solo r fra 0 e L/2 per la gofr*/
        }
  ...
  if (_measure_pressure){
    pressure_temp = _ptail + (_rho*_measurement(_index_temp)) + ((48/(3.*_volume)) * pressure_temp);
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv){
    _measurement(_index_cv) = (tenergy_temp * _npart)*(tenergy_temp * _npart);
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
...
void System :: averages(int blk){
  ...
  if(_measure_cv) _average(_index_cv) = (_beta * _beta) * (_average(_index_cv) - (_average(_index_tenergy) * _average(_index_tenergy) * _npart * _npart)) / _npart;
  if(_measure_magnet) _average(_index_magnet) /= _npart;
  ...
  if (_measure_tenergy){
    ...
    //Versione modificata da me per Es6
    /*average  = _average(_index_tenergy);
    sum_average = _global_av(_index_tenergy);
    sum_ave2 = _global_av2(_index_tenergy);
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if (_measure_gofr){     /*per ogni bin faccio media a blocchi, all'ultimo blocco plotto tutto l'istogramma praticamente*/
    for(int i=0; i<_n_bins; i++){
      ...
      if(blk == _nblocks){  /*segno risultati solo all'ultimo blocco*/
        if(_sim_type==1){
          coutf.open("OUTPUT/fin_gofr1.dat",ios::app);
        }
        if(_sim_type==0){
...
double System :: get_EnMeasure(int e){
  if(e==0) return _measurement(_index_tenergy);
  if(e==1) return _measurement(_index_penergy);
  if(e==2) return _measurement(_index_kenergy);
  else {
    cerr << "Errore: tipo di energia invalido passato a gen_EnMeasure()" << endl;
    return -1;
  }
}
